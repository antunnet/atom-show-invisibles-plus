...
module.exports={
	...
	activate: function(state) {
		...
		for (var character in this.invisibles)
		{
			...
			this.invisibles[character].flags=index;
			this.classes[index]=classes;
			
			for (var i=0;i<=this.invisibles[character].classes.length-1;i++)
			{
				var class1=this.invisibles[character].classes[i];
				if (class2characters[class1]==null)
				{
					class2characters[class1]=[];
				}
				class2characters[class1].push(character);
			}
		}
		...
		this.subscription.add(
		    atom.styles.addStyleSheet(
		        atom.themes.loadStylesheet(stylePath,true),
		        { sourcePath: stylePath })
		);
		var styleSheet;
		for (var i=document.styleSheets.length-1;i>=0;i--)
		{
			if (document.styleSheets[i].ownerNode!=null &&
			    document.styleSheets[i].ownerNode.sourcePath==stylePath)
			{
		...
		(function(keyPath,config,callback) {
			...
			if (config.content!=null)
			{
				var updater=() => {
					...
					for (var character of class2characters[config.content])
					{
			...
			if (config.contents!=null)
			{
				var updater=() => {
					for (var i=0;i<=config.contents.length-1;i++)
					{
						...
						for (var character of 
						     (class2characters[config.contents[i]] || []))
						{
							this.invisibles[character].placeholder=
							    (placeholder!=character && placeholder!=" "
							    ? placeholder : null);
			...
			{
				var updater=() => {
					for (var i=0;i<=styleSheet.cssRules.length-1;i++)
					{
...
//					this.currentBuiltInClassNameFlags|=(1 << 1);
					...
					this.currentBuiltInClassNameFlags|=(
					    showInvisiblesPlus.invisibles[nextCharacter].flags
					    << 3);
				}
			};
