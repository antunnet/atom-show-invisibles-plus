...
module.exports={
		...
		borders: {
			...
			properties: {
				...
				controlCharacter: {
					...
					description: "Show border with error color for control character, such as \\a, \\t and \\v.",
					type: "boolean",
					default: true,
					order: 3,
	...
	activate: function(state) {
		...
		// Hack to hide grammar settings
		var package=atom.packages.getLoadedPackage("show-invisibles-plus");
		for (var i=0;i<=package.grammars.length-1;i++)
		{
			package.grammars[i].path="";
		}
		...
		(function(keyPath,config,callback) {
			...
			if (config.sync!=null)
			{
				...
				this.subscription.add(
					...
					atom.config.set(config.sync,value);
				}));
			}
			if (config.content!=null)
			{
				...
				this.subscription.add(
					...
					if (value.length>1)
					{
						atom.config.set(keyPath,undefined);
					}
					else
					{
						updater(value);
		...
		function updateStyle(clazz,style,value)
		{
			for (var i=0;i<=document.styleSheets.length-1;i++)
			{
				for (var j=0;j<=document.styleSheets[i].cssRules.length-1;j++)
				{
					...
					if (cssRule.selectorText=="atom-text-editor::shadow "+
					    ".invisibles-plus."+clazz)
					{
